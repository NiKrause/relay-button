...
import { parseDeployPlan } from '../src/deploy-plan.ts'
...
test('parseDeployPlan parses integer, boolean, and JSON overrides', () => {
  const plan = parseDeployPlan({
    ...
    ALEPH_VM_REQUIRED_PORTS_JSON: '[{"port":22,"tcp":true,"udp":false,"purpose":"SSH"}]'
  })
  ...
  assert.deepEqual(plan.requiredPorts, [{ port: 22, tcp: true, udp: false, purpose: 'SSH' }])
})
